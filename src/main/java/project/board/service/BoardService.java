...
import project.board.repository.BoardFileRepository;
...
public class BoardService {
    ...
    public void save(BoardDTO boardDTO) throws IOException { //boardDTO -> boardEntity로
        ...
        if (boardDTO.getBoardFile().isEmpty()) {
            ...
            BoardEntity board = boardRepository.findById(saveId).get(); // 첨부파일(자식)이 여러개이므로 부모 객체 먼저 호출

            for (MultipartFile boardFile : boardDTO.getBoardFile()) {
//                MultipartFile boardFile = boardDTO.getBoardFile(); // 1. -> 반복문으로 꺼내므로 불필요
    ...
    public Page<BoardDTO> paging(Pageable pageable) {
        ...
        Page<BoardDTO> boardDTOS = boardEntities.map(
                board -> new BoardDTO(board.getId(), board.getBoardWriter(), board.getBoardTitle(), board.getBoardHits(), board.getCreatedTime()));
        return boardDTOS;
    }
}
